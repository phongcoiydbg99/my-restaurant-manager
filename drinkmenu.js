...
export default class drinkmenu extends Component {
  constructor(props){
    ...
    this.state ={
      ...
      //state newDrink de luu lai thong tin do uong vua moi nhap
      
    };
  }
  componentDidMount(){
    axios.get('http://localhost:8080/dish/category/MainCourse').then(res => this.setState({result: res}));
  }
  ...
  saveNewData(){
    //ham nay se duoc invoke khi save du lieu moi
     axios.post('http://localhost:8080/dish/add',data).then(res => console.log(res)).then(this.setState({newDrink: data}));
